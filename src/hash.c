// Copyright (c) 2019 Roland Bernard

#include "hash.h"

hash_t hash_crc(const char* data, len_t size, const hash_t* table) {
	hash_t hash = ~0;

	for(len_t i = 0; i < size; i++)
		hash = (hash >> 8) ^ table[(hash ^ data[i]) & 0xFF];

	return hash ^ ~0;
}

#define FNV_OFFSET_BASIS 2166136261
#define FNV_PRIME 16777619

hash_t hash_fnv(const char* data, len_t size) {
	hash_t hash = FNV_OFFSET_BASIS;

	for(len_t i = 0; i < size; i++) {
		hash ^= data[i];
		hash *= FNV_PRIME;
	}

	return hash;
}

#define ROTR256(X, N) ((X >> N) | (X << (32-N)))

#define CH256(x,y,z) ((x & y) ^ (~x & z))
#define MAJ256(x,y,z) ((x & y) ^ (x & z) ^ (y & z))
#define EP0256(x) (ROTR256(x,2) ^ ROTR256(x,13) ^ ROTR256(x,22))
#define EP1256(x) (ROTR256(x,6) ^ ROTR256(x,11) ^ ROTR256(x,25))
#define SIG0256(x) (ROTR256(x,7) ^ ROTR256(x,18) ^ (x >> 3))
#define SIG1256(x) (ROTR256(x,17) ^ ROTR256(x,19) ^ (x >> 10))

static const uint32_t sha_k256[64] = {
	0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,0x923f82a4,0xab1c5ed5,
	0xd807aa98,0x12835b01,0x243185be,0x550c7dc3,0x72be5d74,0x80deb1fe,0x9bdc06a7,0xc19bf174,
	0xe49b69c1,0xefbe4786,0x0fc19dc6,0x240ca1cc,0x2de92c6f,0x4a7484aa,0x5cb0a9dc,0x76f988da,
	0x983e5152,0xa831c66d,0xb00327c8,0xbf597fc7,0xc6e00bf3,0xd5a79147,0x06ca6351,0x14292967,
	0x27b70a85,0x2e1b2138,0x4d2c6dfc,0x53380d13,0x650a7354,0x766a0abb,0x81c2c92e,0x92722c85,
	0xa2bfe8a1,0xa81a664b,0xc24b8b70,0xc76c51a3,0xd192e819,0xd6990624,0xf40e3585,0x106aa070,
	0x19a4c116,0x1e376c08,0x2748774c,0x34b0bcb5,0x391c0cb3,0x4ed8aa4a,0x5b9cca4f,0x682e6ff3,
	0x748f82ee,0x78a5636f,0x84c87814,0x8cc70208,0x90befffa,0xa4506ceb,0xbef9a3f7,0xc67178f2
};

void hash_sha256(hash256_t ret, const char* data, len_t size) {
	uint32_t hash[8] = {
		0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
		0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19
	};
	uint64_t bitsize = size*8;

	for(len_t i = 0; i < size+9; i+=64) {
		uint8_t chunk[64] = { 0 };
		uint32_t w[64] = { 0 };
		len_t j;
		for(j = 0; j < 64 && i+j < size; j++)
			chunk[j] = data[i+j];
		if(j < 64) {
			if(j+i == size) {
				chunk[j] = 0x80;
				j++;
			}
			if(j < 56) {
				chunk[63] = bitsize & 0xff;
				chunk[62] = (bitsize >> 8) & 0xff;
				chunk[61] = (bitsize >> 16) & 0xff;
				chunk[60] = (bitsize >> 24) & 0xff;
				chunk[59] = (bitsize >> 32) & 0xff;
				chunk[58] = (bitsize >> 40) & 0xff;
				chunk[57] = (bitsize >> 48) & 0xff;
				chunk[56] = (bitsize >> 56) & 0xff;
			}
		}
		for(j = 0; j < 16; j++)
			w[j] = (chunk[4*j] << 24) | (chunk[4*j+1] << 16) | (chunk[4*j+2] << 8) | chunk[4*j+3];
		for(; j < 64; j++)
			w[j] = w[j-16] + SIG0256(w[j-15]) + w[j-7] + SIG1256(w[j-2]);

		uint32_t a = hash[0];
		uint32_t b = hash[1];
		uint32_t c = hash[2];
		uint32_t d = hash[3];
		uint32_t e = hash[4];
		uint32_t f = hash[5];
		uint32_t g = hash[6];
		uint32_t h = hash[7];
		for(j = 0; j < 64; j++) {
			uint32_t temp1 = h + EP1256(e) + CH256(e, f, g) + sha_k256[j] + w[j];
			uint32_t temp2 = EP0256(a) + MAJ256(a, b, c);

			h = g;
			g = f;
			f = e;
			e = d + temp1;
			d = c;
			c = b;
			b = a;
			a = temp1 + temp2;
		}
		hash[0] += a;
		hash[1] += b;
		hash[2] += c;
		hash[3] += d;
		hash[4] += e;
		hash[5] += f;
		hash[6] += g;
		hash[7] += h;
	}

	for(len_t i = 0; i < 8; i++)
		for(len_t j = 0; j < 4; j++)
			ret[4*i+j] = (hash[i] >> (24 - 8*j)) & 0xff;
}

#define ROTR512(X, N) ((X >> N) | (X << (64-N)))

#define CH512(x,y,z) ((x & y) ^ (~x & z))
#define MAJ512(x,y,z) ((x & y) ^ (x & z) ^ (y & z))
#define EP0512(x) (ROTR512(x,28) ^ ROTR512(x,34) ^ ROTR512(x,39))
#define EP1512(x) (ROTR512(x,14) ^ ROTR512(x,18) ^ ROTR512(x,41))
#define SIG0512(x) (ROTR512(x,1) ^ ROTR512(x,8) ^ (x >> 7))
#define SIG1512(x) (ROTR512(x,19) ^ ROTR512(x,61) ^ (x >> 6))

static const uint64_t sha_k512[80] = {
	0x428a2f98d728ae22, 0x7137449123ef65cd, 0xb5c0fbcfec4d3b2f, 0xe9b5dba58189dbbc, 0x3956c25bf348b538,
	0x59f111f1b605d019, 0x923f82a4af194f9b, 0xab1c5ed5da6d8118, 0xd807aa98a3030242, 0x12835b0145706fbe,
	0x243185be4ee4b28c, 0x550c7dc3d5ffb4e2, 0x72be5d74f27b896f, 0x80deb1fe3b1696b1, 0x9bdc06a725c71235,
	0xc19bf174cf692694, 0xe49b69c19ef14ad2, 0xefbe4786384f25e3, 0x0fc19dc68b8cd5b5, 0x240ca1cc77ac9c65,
	0x2de92c6f592b0275, 0x4a7484aa6ea6e483, 0x5cb0a9dcbd41fbd4, 0x76f988da831153b5, 0x983e5152ee66dfab,
	0xa831c66d2db43210, 0xb00327c898fb213f, 0xbf597fc7beef0ee4, 0xc6e00bf33da88fc2, 0xd5a79147930aa725,
	0x06ca6351e003826f, 0x142929670a0e6e70, 0x27b70a8546d22ffc, 0x2e1b21385c26c926, 0x4d2c6dfc5ac42aed,
	0x53380d139d95b3df, 0x650a73548baf63de, 0x766a0abb3c77b2a8, 0x81c2c92e47edaee6, 0x92722c851482353b,
	0xa2bfe8a14cf10364, 0xa81a664bbc423001, 0xc24b8b70d0f89791, 0xc76c51a30654be30, 0xd192e819d6ef5218,
	0xd69906245565a910, 0xf40e35855771202a, 0x106aa07032bbd1b8, 0x19a4c116b8d2d0c8, 0x1e376c085141ab53,
	0x2748774cdf8eeb99, 0x34b0bcb5e19b48a8, 0x391c0cb3c5c95a63, 0x4ed8aa4ae3418acb, 0x5b9cca4f7763e373,
	0x682e6ff3d6b2b8a3, 0x748f82ee5defb2fc, 0x78a5636f43172f60, 0x84c87814a1f0ab72, 0x8cc702081a6439ec,
	0x90befffa23631e28, 0xa4506cebde82bde9, 0xbef9a3f7b2c67915, 0xc67178f2e372532b, 0xca273eceea26619c,
	0xd186b8c721c0c207, 0xeada7dd6cde0eb1e, 0xf57d4f7fee6ed178, 0x06f067aa72176fba, 0x0a637dc5a2c898a6,
	0x113f9804bef90dae, 0x1b710b35131c471b, 0x28db77f523047d84, 0x32caab7b40c72493, 0x3c9ebe0a15c9bebc,
	0x431d67c49c100d4c, 0x4cc5d4becb3e42b6, 0x597f299cfc657e2a, 0x5fcb6fab3ad6faec, 0x6c44198c4a475817
};

void hash_sha512(hash512_t ret, const char* data, len_t size) {
	uint64_t hash[8] = {
		0x6a09e667f3bcc908, 0xbb67ae8584caa73b, 0x3c6ef372fe94f82b, 0xa54ff53a5f1d36f1,
		0x510e527fade682d1, 0x9b05688c2b3e6c1f, 0x1f83d9abfb41bd6b, 0x5be0cd19137e2179
	};
	uint64_t bitsize = size*8;

	for(len_t i = 0; i < size+17; i+=128) {
		uint8_t chunk[128] = { 0 };
		uint64_t w[80] = { 0 };
		len_t j;
		for(j = 0; j < 128 && i+j < size; j++)
			chunk[j] = data[i+j];
		if(j < 128) {
			if(j+i == size) {
				chunk[j] = 0x80;
				j++;
			}
			if(j < 112) {
				chunk[127] = bitsize & 0xff;
				chunk[126] = (bitsize >> 8) & 0xff;
				chunk[125] = (bitsize >> 16) & 0xff;
				chunk[124] = (bitsize >> 24) & 0xff;
				chunk[123] = (bitsize >> 32) & 0xff;
				chunk[122] = (bitsize >> 40) & 0xff;
				chunk[121] = (bitsize >> 48) & 0xff;
				chunk[120] = (bitsize >> 56) & 0xff;
			}
		}
		for(j = 0; j < 16; j++)
			w[j] = ((uint64_t)chunk[8*j] << 56) | ((uint64_t)chunk[8*j+1] << 48) |
				((uint64_t)chunk[8*j+2] << 40) | ((uint64_t)chunk[8*j+3] << 32) |
				((uint64_t)chunk[8*j+4] << 24) | ((uint64_t)chunk[8*j+5] << 16) |
				((uint64_t)chunk[8*j+6] << 8) | (uint64_t)chunk[8*j+7];
		for(; j < 80; j++)
			w[j] = w[j-16] + SIG0512(w[j-15]) + w[j-7] + SIG1512(w[j-2]);

		uint64_t a = hash[0];
		uint64_t b = hash[1];
		uint64_t c = hash[2];
		uint64_t d = hash[3];
		uint64_t e = hash[4];
		uint64_t f = hash[5];
		uint64_t g = hash[6];
		uint64_t h = hash[7];
		for(j = 0; j < 80; j++) {
			uint64_t temp1 = h + EP1512(e) + CH512(e, f, g) + sha_k512[j] + w[j];
			uint64_t temp2 = EP0512(a) + MAJ512(a, b, c);

			h = g;
			g = f;
			f = e;
			e = d + temp1;
			d = c;
			c = b;
			b = a;
			a = temp1 + temp2;
		}
		hash[0] += a;
		hash[1] += b;
		hash[2] += c;
		hash[3] += d;
		hash[4] += e;
		hash[5] += f;
		hash[6] += g;
		hash[7] += h;
	}

	for(len_t i = 0; i < 8; i++)
		for(len_t j = 0; j < 8; j++)
			ret[8*i+j] = (hash[i] >> (56 - 8*j)) & 0xff;
}
