...
error_t client_main(const config_t conf) {
	...
	if(use_udp && use_dis) {
		...
		while(i) {
			char buffer[START_BUFFER_LEN] = "HI";
			int len = sendto(udp_sock, buffer, 2, 0, (struct sockaddr*)&saddr, sizeof(saddr));
			if(len == 2) {
				unsigned int raddr_len = sizeof(raddr);
				len = recvfrom(udp_sock, buffer, START_BUFFER_LEN, 0, (struct sockaddr*)&raddr, &raddr_len);
			...
			i--;
			len = read(STDIN_FILENO, buffer, START_BUFFER_LEN);
			for(int j = 0; j < len; j++)
	...
	while(!end) {
		...
		char tmp_in[START_BUFFER_LEN];
		if(use_group)
			snprintf(tmp_in, START_BUFFER_LEN, "@%s: %s", conf.group, status);
		else
			snprintf(tmp_in, START_BUFFER_LEN, "no group: %s", status);
			...
			if(len_in >= 1) {
				for(int i = 0; i < len_in; i++) {
					if(buff_len < START_BUFFER_LEN) {
						if(tmp_in[i] == '\b' || tmp_in[i] == '\x7f') /* delete */ {
						...
						} else if(tmp_in[i] == '\t' && buff_len < START_BUFFER_LEN) /* tab */ {
							...
							if((isprint(ch) || num_byte >= 2) && buff_len+num_byte <= START_BUFFER_LEN) /* normal char and utf-8 (I have not found a way to check them for printability) */ {
